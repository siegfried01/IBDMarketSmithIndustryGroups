...
Module MarketSmithIndustryGroupsMainProgram
                                      ...
                                      <Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
                                          ...
                                          <Worksheet ss:Name="Export">
                                              ...
                                              <Table ss:ExpandedColumnCount="40" ss:ExpandedRowCount="198" x:FullColumns="1"
                                                  ...
                                                  <Row ss:AutoFitHeight="0" ss:Height="160" ss:StyleID="s64">
    ...
    Private Sub Main2()
        ...
        For Each fileName In Directory.GetFiles(System.Environment.ExpandEnvironmentVariables("%USERPROFILE%\Downloads"), "*.csv")
            ...
            If filesAlreadyLoaded.Contains(fileName) Then Continue For
            Dim name = fileName.Substring(0, fileName.Length - 4)
        ...
        Dim styleNames As HashSet(Of String) = New HashSet(Of String) ' names of styles to be used for color coding cells
                    ...
                    For Each stock In stocksInCurrentIndustryGroup
                        ...
                        If count <= topMemberCount Then
                            ...
                            If annotations.Contains("X") Then
                                hrefStyle = hrefStyleStrikeThru ' For extended stocks
                            Else
                            ...
                            If annotations.Contains("X") Then ' X is used by the Extended list which uses the strike thru font
                                font = fonts(1)
                            Else
                                font = fonts(0)
                            End If
    ...
    Private Function RestoreFromXML() As List(Of StockList)
        Dim favoriteStocksList = New List(Of StockList)
        Dim fn As String = FindFavoriteStockListFile()
        Dim favoritesList = System.IO.File.ReadAllText(fn)
        Dim xmlSerializer As System.Xml.Serialization.XmlSerializer = New System.Xml.Serialization.XmlSerializer(favoriteStocksList.GetType)
        Dim ns As New System.Xml.Serialization.XmlSerializerNamespaces
        Dim string_reader As New System.IO.StringReader(favoritesList)
        favoriteStocksList = xmlSerializer.Deserialize(string_reader)
        For Each favorite In favoriteStocksList
            ...
            If fileNameFavoritesMap.ContainsKey((stockList.Name)) Then
                If fileNameFavoritesMap(stockList.Name).CsvFileFoundAndLoaded Then
                    ...
                    If String.IsNullOrEmpty(stockList.Attributes.DisplayName) Then
                        stockListName = stockList.Name.Trim
                    Else
                        stockListName = stockList.Attributes.DisplayName.Trim
                    End If
